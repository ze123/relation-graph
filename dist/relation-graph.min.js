...
/* harmony default export */ var SeeksRGLink = ({
  ...
  // },
  methods: {
	// 2021.01.15 依据象限调整箭头位置，手动画的箭头！
	getPathTransform(thisRelation) {
		// console.log(thisRelation.text.length*20)
		// 一个字20间隔,我简直太厉害了哈哈哈哈!
	  var l = -thisRelation.text.length*20,angletype=thisRelation.textPositon.angletype;
	  if (angletype == 1 || angletype == 2) {
		  return `M${-16+l} 7 L${-4+l} 0 L${-4+l} 14 Z`;
		// return "M-54 7 L-42 0 L-42 14 Z";
	  } else {
	    return "M-16 -7 L-4 -14 L-4 0 Z";
	  }
	},
    ...
    createLinePath(from, to, ri, relationData) {
      ...
      if (__lineShape === 4) {
		...
		// 2021.01.15 区分text与path，偏函数+获取象限
		var fn=core4vue_SeeksGraphMath.getTextAngle(fx, fy, tx, ty);
		var tfn=fn('text');
        relationData.textPositon.rotate = tfn.tan;
        relationData.textPositon.rotate2 = fn('path').tan;
		var realangle = tfn.position;
        relationData.textPositon.angletype = realangle;
        var _xxx = _angle_type === 2 || _angle_type === 4 ? -1 : 1;
        var _yyy = _angle_type === 1 || _angle_type === 3 ? 1 : -1;
		// var _x = (Math.sin(Math.atan(__buff_x / __buff_y)) * 10 / Math.sin(90)) * _xxx;
		// var _y = Math.sin(Math.atan(__buff_y / __buff_x)) * 10 / Math.sin(90);
		// relationData.textPositon.x = parseInt(__start.x + __buff_x / 2 - _x);
		// relationData.textPositon.y = parseInt(__start.y + __buff_y / 2 - _y);
		
        var _x = (Math.sin(Math.atan(__buff_x / __buff_y)) * 10 / Math.sin(90)) * _xxx;
        var _y = Math.sin(Math.atan(__buff_y / __buff_x)) * 10 / Math.sin(90);
		var tempx = parseInt(__start.x + __buff_x / 2 - _x)
		var tempy = parseInt(__start.y + __buff_y / 2 - _y);
		// if(realangle == 1 || realangle == 4){
		// 	tempy = tempy - 5;
		// }else if(realangle == 2 || realangle == 3){
		// 	tempx = tempx + 15;
		// 	tempy = tempy + 5;
		// }
		relationData.textPositon.x = tempx;
		relationData.textPositon.y = tempy;
		// console.log("__start.y",__start.y)
		// console.log("__buff_y",__buff_y)
		// console.log("_y",_y)
		// console.log("teytPositon",__start.y + __buff_y / 2 - _y)
...
var SeeksRGLink_render = function () {
  ...
  return _vm.lineProps.isHide !== true && _vm.isAllowShowNode(_vm.lineProps.fromNode) && _vm.isAllowShowNode(_vm.lineProps.toNode) ? _c("g", { ref: "seeksRGLink", attrs: { transform: "translate(0,0)" } }, [_vm._l(_vm.lineProps.relations, function (thisRelation, ri) {
	return [thisRelation.isHide === false ? _c("g", { key: "l-" + thisRelation.id }, [_c("path", {
	  ...
	  key: "r-" + thisRelation.id,
	  staticClass: "c-rg-link-rect",
	  style: {
	    fill: 'yellow'
	  },
	  attrs: {
		x: 0,
		y: -20,
		width: thisRelation.text.length*20,
		height: 26,
	    stroke: "#bbb",
		transform: _vm.getTextTransform(thisRelation, thisRelation.textPositon.x - 10, thisRelation.textPositon.y + 8, thisRelation.textPositon.rotate)
	  },
	}),
	_c("text", {
      ...
      attrs: { x: 0, y: 0, transform: _vm.getTextTransform(thisRelation, thisRelation.textPositon.x - 10, thisRelation.textPositon.y + 8, thisRelation.textPositon.rotate)},
      on: {
        click: function ($event) {
          return _vm.onClick($event);
        }
	...
	// 2021.01.15 新增箭头
	_c("path", {
	  key: "p-" + thisRelation.id,
	  staticClass: "c-rg-link-path",
	  style: {
	    fill: 'yellow'
	  },
	  attrs: {
		// 2021.01.15 依据象限调整箭头位置，手动画的箭头！
	    d: _vm.getPathTransform(thisRelation),
	    stroke: "#bbb",
	    fill: "none",
		transform: _vm.getTextTransform(thisRelation, thisRelation.textPositon.x - 10, thisRelation.textPositon.y + 8, thisRelation.textPositon.rotate2)
	  },
	})]) : _vm._e()]) : _vm._e()];
  })], 2) : _vm._e();
};
